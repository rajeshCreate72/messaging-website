...
export const syncingMessagesSuccess = (isSuccessMsg) => ({
  type: MESSAGES_SYNC_SUCCESS,
  payload: isSuccessMsg,
});

export const syncingMessagesFailed = (error) => ({
...
export const fetchMessages = (chatUsers) => async (dispatch) => {
  ...
  try {
    const response = await axios.get("http://localhost:8000/api/chats", {
      params: { userId: chatUsers.userId, contactId: chatUsers.contactId },
    });
