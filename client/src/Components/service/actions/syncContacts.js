...
export const postContact = (contact) => async (dispatch) => {
    ...
    try {
        const response = await axios.post("https://chat-website-server.vercel.app/api/contacts", contact, {
...
export const getContacts = (ofUser) => async (dispatch) => {
    ...
    console.log(ofUser);
    try {
        const response = await axios.get("https://chat-website-server.vercel.app/api/contacts", {
